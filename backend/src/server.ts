    ...
    if (privateRoute) {
      ...
      if (!token) {
        return res
          ...
          .json({
            error:
              "Acesso restrito! É necessário autenticar-se no sistema para prosseguir.",
          });
      }
    }
    next();
    ...
    this.routes.get().forEach((route: any) => {
      switch (route.method) {
        case "get":
          ...
          return;
        case "post":
          this.express.post(route.path, route.handler);
          return;
        case "delete":
          this.express.delete(route.path, route.handler);
          return;
      }
    });
